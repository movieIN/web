...
//					request.setAttribute("inv_type", project.getInv_type());
		...
		} else if ("listOneProject".equals(request.getParameter("action"))) {
			dao = new ProjectDAO();
			int seq_PID = Integer.parseInt(request.getParameter("seq_PID"));
			project = dao.listOneProject(seq_PID);
			request.setAttribute("project", project);
			url = "/projectList/proj" + seq_PID + ".jsp";
		} else if ("browseProject".equals(request.getParameter("action"))) {
			url="/browseproject.jsp";
			dao = new ProjectDAO();
			ArrayList <FilterVO> filterList = null;
			ArrayList <FilterVO> tempList = null;
			if (request.getParameter("sorting") == null) {
				url = url + "?sorting=cnt desc";	
				filterList = dao.listFilter("cnt desc");
			} else {
				url = url + "?sorting=" + request.getParameter("sorting");
				filterList = dao.listFilter(request.getParameter("sorting"));
			}
			String inv_typeArray[] = request.getParameterValues("inv_type");
			String genreArray[] = request.getParameterValues("genre");
			String purposeArray[] = request.getParameterValues("purpose");
			if (inv_typeArray != null) {
				for (int i = 0 ; i < inv_typeArray.length; ++i) {
					request.setAttribute(inv_typeArray[i], i);
				}
				tempList = new ArrayList<FilterVO>();
				for (int i = 0; i < filterList.size(); ++i) {
					for (int j = 0; j < inv_typeArray.length; ++j) {
						if (inv_typeArray[j].equals(filterList.get(i).getInv_type())) {
			...
			if (genreArray != null) {
				for (int i = 0 ; i < genreArray.length; ++i) {
					request.setAttribute(genreArray[i], i);
				}
				tempList = new ArrayList<FilterVO>();
				for (int i = 0; i < filterList.size(); ++i) {
					for (int j = 0; j < genreArray.length; ++j) {
						if (genreArray[j].equals(filterList.get(i).getGenre1())) {
							tempList.add(filterList.get(i));
							break;
						} else if (genreArray[j].equals(filterList.get(i).getGenre2())) {
							tempList.add(filterList.get(i));
							break;
						} else if (genreArray[j].equals(filterList.get(i).getGenre3())) {
							tempList.add(filterList.get(i));
							break;
						}
					}
			...
			if (purposeArray != null) {
				for (int i = 0 ; i < purposeArray.length; ++i) {
					request.setAttribute(purposeArray[i], i);
				}
				tempList = new ArrayList<FilterVO>();
				for (int i = 0; i < filterList.size(); ++i) {
					for (int j = 0; j < purposeArray.length; ++j) {
						if (purposeArray[j].equals(filterList.get(i).getPurpose())) {
							tempList.add(filterList.get(i));
							break;
						}
					}
		...
		/*else if ("search".equals(request.getParameter("action"))) {
			request.setAttribute("search", dao.search(request.getParameter("q")));
			url="/project/search.jsp";
		}*/
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("logOn") != null) {
			...
			if ("insertProject".equals(request.getParameter("action"))) {
				...
				url = "index.jsp";
			} else if ("insertInvestment".equals(request.getParameter("action"))) {
				dao.insertInvest(Integer.parseInt(request.getParameter("seq_PID")), logOn.getId(), Integer.parseInt(request.getParameter("investment")));
				url="/index.jsp";
			}
		} else {
			url = "login.jsp";
